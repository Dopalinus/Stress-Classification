...
- **Data** - Data folder with Fluorecence data, Atlas folder with Allen Brain Atlas related files for organization.
...
- Hardware: Dell Precision 3650 with Intel Core i7-10700 (8 cores). 32 GB of RAM.
- Packages as defined at the end of README

## Installation Guide
- Download main directory 'cFos_pipeline' file to a editable directory (as the code makes writes and can be blocked)
...
- Once complete run succeeds, switch to config.return_classifyDict_default() to perform published analysis. This can take on the order of 8 to 14 hours.

## Running on your own data
- Key considerations will include generating a pandas dataframe with the correct columns and data for each, reference the output of 'initFunctions.py' for this format.
...
- specific classifications must be defined in 'helperFunctions.create_drugClass_dict()' which creates the dictionary of analyses which can be called, and which 'stressor' entities are included and translated to classes. Classes can be single stressor, timepoint or drug.
